...
require "fileutils"
require "action_icon/sync/transformations"
  ...
  module Sync
    class ProcessVariants < Rails::Generators::Base
      ...
      def process
        ...
        excluded_variants = ActionIcon.configuration.libraries.dig(@name.to_sym)&.exclude_variants || []
        ...
        @library[:variants].each do |variant_name, variant_source_path|
          ...
          raise "[Action Icon] Failed to find the icons directory: '#{source}'" unless Dir.exist?(source)
        ...
        say "[Action Icon] Icon variants processed successfully"
      end
      ...
      def remove_previously_downloaded(variants)
        variants.each do |variant|
          FileUtils.rm_rf(File.join(ActionIcon.configuration.destination_path, @name, variant.to_s))
        end
      end

      def transformations
        ActionIcon.libraries.dig(@name.to_sym)&.try(:transformations) || {}
      end
    end
  end
end
