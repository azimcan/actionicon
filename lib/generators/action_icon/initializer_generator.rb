...
module ActionIcon
  class InitializerGenerator < Rails::Generators::Base
    ...
    desc "Create the Action Icon initializer."
    ...
    def insert_default_configuration
      ...
      if libraries.present?
        ...
        insert_into_file INITIALIZER, default_configuration, after: "ActionIcon.configure do |config|\n"
      end
    end

    def insert_custom_destination_path
      return if options[:destination] && options[:destination] == ActionIcon.configuration.destination_path

      insert_into_file INITIALIZER, <<~RB.indent(2), after: "ActionIcon.configure do |config|\n"
    ...
    INITIALIZER = "config/initializers/action_icon.rb"
    ...
    def library_configuration
      libraries.map { ActionIcon.libraries[_1.to_sym].initializer_config }.join("\n")
    end
